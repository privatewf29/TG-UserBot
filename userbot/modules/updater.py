    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await event.edit(
                ...
                ' untuk dapat deploy perubahan terbaru dari TG-UserBot.`'
            )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await event.edit(
                f'{txt}\n`Kredensial Heroku tidak valid untuk deploy TG-UserBot dyno.`'
            )
            return repo.__del__()
        await event.edit('`[HEROKU]:'
                         '\nDyno TG-UserBot Sedang Dalam Proses, Mohon Menunggu 7-8 Menit`'
                         )
        ups_rem.fetch(ac_br)
        ...
        if build.status == "failed":
            ...
            await event.edit("`TG-UserBot Berhasil Di Deploy!\n" "Restarting, Mohon Menunggu ....`")
